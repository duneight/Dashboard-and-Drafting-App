...
async function upsertLeague(leagueKey: string, gameId: string, season: string, leagueData: LeagueData) {
  ...
  return await prisma.league.upsert({
    where: { leagueKey },
      ...
      password: leagueInfo.password,
            draftStatus: leagueInfo.draft_status,
            numTeams: parseInt(leagueInfo.num_teams),
      editKey: leagueInfo.edit_key,
      weeklyDeadline: leagueInfo.weekly_deadline,
      leagueUpdateTimestamp: leagueInfo.league_update_timestamp,
      scoringType: leagueInfo.scoring_type,
      leagueType: leagueInfo.league_type,
      renew: leagueInfo.renew,
      renewed: leagueInfo.renewed,
      irisGroupChatId: leagueInfo.iris_group_chat_id,
      allowAddToDlExtraPos: leagueInfo.allow_add_to_dl_extra_pos ? parseInt(leagueInfo.allow_add_to_dl_extra_pos) : null,
      isProLeague: leagueInfo.is_pro_league === '1',
      isCashLeague: leagueInfo.is_cash_league === '1',
      currentMatchupPeriod: leagueInfo.current_matchup_period ? parseInt(leagueInfo.current_matchup_period) : null,
            isFinished: leagueInfo.is_finished === '1',
      isApiFootball: leagueInfo.is_api_football === '1',
      apiSelections: leagueInfo.api_selections,
      season,
          },
          create: {
      leagueKey,
  ...
  for (const team of teams) {
    const standing = standingsMap.get(team.team_key)
      ...
      where: { teamKey: team.team_key },
            update: {
        name: team.name,
        isOwnedByCurrentLogin: team.is_owned_by_current_login === '1',
        url: team.url,
        teamLogos: team.team_logos ? JSON.stringify(team.team_logos) : null,
        waiverPriority: team.waiver_priority ? parseInt(team.waiver_priority) : null,
        ...
        leagueScoringType: team.league_scoring_type,
        managers: team.managers ? JSON.stringify(team.managers) : null,
        managerNickname: team.managers?.manager?.nickname || null,
        managerGuid: team.managers?.manager?.guid || null,
        managerEmail: team.managers?.manager?.email || null,
        managerImageUrl: team.managers?.manager?.image_url || null,
        managerIsCommissioner: team.managers?.manager?.is_commissioner === '1',
        managerIsCurrentLogin: team.managers?.manager?.is_current_login === '1',
