...
import { Crown, Skull, Search, Info, AlertTriangle, Filter } from "lucide-react";

/**
 * Wall of Shame — Drop‑in Page
...
function Medallion({ name, avatarUrl, rank }: { name: string; avatarUrl?: string; rank: 1 | 2 | 3 }) {
  const ring = rank === 1 ? "from-red-400 to-rose-500" : rank === 2 ? "from-zinc-300 to-zinc-400" : "from-red-800 to-red-900";
    ...
    <div className="sticky top-0 z-20 backdrop-blur supports-[backdrop-filter]:bg-background/60 bg-background/90 border-b border-foreground/10">
      <div className="mx-auto max-w-6xl px-4 py-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-2">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-500 to-rose-500 rounded-full">
            <Skull className="h-5 w-5 text-white" />
            <span className="text-lg font-black text-white">Wall of Shame</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
...
export default function WallOfShamePage() {
  ...
  const { 
    ...
    refetch,
    dataUpdatedAt 
  } = useQuery<WallOfShameResponse>({
    queryKey: ['wall-of-shame'],
    queryFn: async () => {
      const response = await fetch('/api/stats/wall-of-shame')
      if (!response.ok) {
        throw new Error('Failed to fetch Wall of Shame data')
      }
      ...
      <div className="min-h-dvh bg-background text-foreground">
        <div className="mx-auto max-w-6xl px-4 py-8">
          <div className="rounded-2xl border border-destructive/30 bg-destructive/10 p-6">
            <p className="font-semibold">Wall of Shame</p>
